...
- **`SCHEMA-TASK.md`**: removed `subtasks`/`parent` from the "NOT in v1" list (D104) — both fields are now part of the v1 Task schema.
- **`TODO-MD-FORMAT.md`**: documented the subtask indented-checkbox format (`  - [ ] ...` under a parent item) and added the parser walk step (D105).
- **Skill ref `adapter-framework.md`**: added a subtask section matching the spec, so the installed skill stays self-contained.
- **`requests/47-subtasks/PLAN.md`**: status `draft` → `done`.
