...
**v0.7.0** released 2026-05-24 — **index hygiene** (#30). New `octopus forget activity <path-or-id>` verb removes an activity from the SQLite index without touching files; `--archive` moves files to `<parent>/_archive/<name>/` instead. Path-or-id auto-detection (works with `/Users/alex/code/foo` or `foo-a3f9`). Archived activities now hidden by default in `list --all`; pass `--include-archived` to surface them. **508 tests passing** (was 489). See [CHANGELOG.md](CHANGELOG.md).

**v0.6.1** released 2026-05-24 — **skill documentation patch.** `skills/octopus/SKILL.md` brought from v0.4.0 up to v0.6.1, reflecting every shipped surface (kind enum, task promotion, adapter framework, three adapters, capture/edit polish).

**v0.6.0** released 2026-05-24 — **capture + edit polish** (#24). Richer `capture` flags (`--due/--scheduled/--start-date/--end-date/--actor/--energy/--owner/--stage` + tags), atomic Obsidian-compatible tag mutations (`--tag/--add-tag/--remove-tag/--clear-tags` with comma/space/repeated input), new `octopus move`/`mv` verb (separates file-move from frontmatter-edit), cascading slug rename (`set --slug` updates all Octopus-managed refs), and `octopus refs find` helper. Explicit-default values now clear instead of reject. **489 tests passing** (was 404).
