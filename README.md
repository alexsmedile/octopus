...
**v0.6.1** released 2026-05-24 — **skill documentation patch.** `skills/octopus/SKILL.md` brought from v0.4.0 up to v0.6.1, reflecting every shipped surface (kind enum, task promotion, adapter framework, three adapters, capture/edit polish). New sections cover the tag flag matrix, slug rename cascade, `set` vs `mv` boundary, and full `capture` flag set. Reference docs corrected for stale rules (kind-forbidden, `octopus rename`). No code changes.
